...
class Empty extends TweetSet {
  ...
  def mostRetweeted: Nothing = throw new NoSuchElementException("The set is empty")
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweetedAcc(most: Tweet): Tweet =
    if (elem.retweets > most.retweets) remove(elem).mostRetweetedAcc(elem)
    else remove(elem).mostRetweetedAcc(most)
